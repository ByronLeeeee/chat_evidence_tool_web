...
try:
    ...
    OCR_ENGINE = PaddleOCR(use_angle_cls=True, lang='ch', show_log=False, use_gpu=False) # lang='ch' 表示中文识别, use_angle_cls=True 开启方向分类
...
def _run_ffmpeg_sync(cmd_list: list[str], log_callback: Optional[Callable[[str], None]] = None) -> Tuple[int, str, str]:
    ...
    if log_callback: log_callback(f"正在执行同步 FFmpeg: {cmd_str}")
    try:
        ...
        if log_callback: log_callback(f"FFmpeg 完成。返回码: {process.returncode}")
...
def extract_single_frame_ffmpeg_sync(
    ...
    if log_callback: log_callback(f"请求从 {video_file_path_obj.name} 提取第 {frame_index} 帧到 {output_frame_path_obj.name}")

    output_dir = output_frame_path_obj.parent
    try:
        output_dir.mkdir(parents=True, exist_ok=True)
    except OSError as e:
         if log_callback: log_callback(f"错误: 创建目录 {output_dir} 失败: {e}")
         return False
    ...
    if log_callback: log_callback(f"单帧提取结果 - 返回码: {return_code}, 文件存在: {frame_exists}")
...
def slice_image_sync(
    ...
    try:
        output_path.mkdir(parents=True, exist_ok=True)
         ...
         if log_callback: log_callback(f"错误: 创建切片输出目录 {output_path} 失败: {e}")
         return []
    ...
    try:
        ...
        if log_callback: log_callback(f"图片尺寸: 宽度={img_width}, 高度={img_height}")
        if log_callback: log_callback(f"裁剪参数: 切片高度={slice_height}, 重叠={overlap}")
        ...
        if img_height <= slice_height: total_steps = 1 # 如果图像比切片高度短，则只有一个切片
        ...
        while start_y < img_height:
            ...
            if log_callback: log_callback(f"  正在裁剪切片 {current_slice_num}/{total_steps}: Y={start_y} 到 Y={end_y}")
        ...
        if log_callback: log_callback(f"错误: 打开或处理图片 {source_image_path} 失败: {open_err}")
...
class OcrFilter:
    ...
    def __init__(self, image_session_folder: str, ocr_engine_instance,
        ...
        self.analysis_rect_tuple = analysis_rect_tuple # 可选的OCR分析区域 (x, y, width, height)
    ...
    def _preprocess_ocr_lines(self, ocr_text_lines: List[str]) -> List[str]:
        ...
        processed = []
        # 如果存在排除列表，预处理以便快速查找
        excluded_set = {ex.strip() for ex in self.exclusion_list if ex.strip()} if self.exclusion_list else set()
    ...
    def _lines_overlap(self, lines1_tail: List[str], lines2_head: List[str]) -> bool:
        """
        检查 lines1 的尾部和 lines2 的头部之间是否存在显著的文本重叠。
        简单检查：至少有一行是共同的。
        您可能需要更复杂的检查，例如，要求多行共同或顺序一致。
        """
        if not lines1_tail or not lines2_head: # 如果任一列表为空，则无重叠
            return False
        # 在 _preprocess_ocr_lines 中已经去除了空格
        set1 = set(lines1_tail)
        set2 = set(lines2_head)
        # self._log(f"    重叠检查: 尾部集合={set1}, 头部集合={set2}, 交集={set1.intersection(set2)}")
        return not set1.isdisjoint(set2) # 如果交集非空，则存在重叠
    ...
    def run_filter(self) -> List[str]:
        ...
        # 存储上一张被保留图像的所有唯一处理后的行集合，用于高效检查“新内容”
        last_kept_processed_lines_set: set[str] = set()
            ...
            for i, img_path in enumerate(image_files):
                ...
                try:
                        ...
                        try:
                            ...
                            if w > 0 and h > 0 and x >= 0 and y >= 0 and \
                               x + w <= pil_img_full.width and y + h <= pil_img_full.height:
                                ...
                                img_cropped = pil_img_full.crop((x, y, x + w, y + h)) # 裁剪图像
                                path_for_ocr = ocr_temp_dir / f"cropped_{img_path.name}" # 更新OCR路径为裁剪后的图像
                                img_cropped.save(path_for_ocr)
                                img_cropped.close() # 关闭裁剪后的图像对象
                            else:
                                self._log(f"警告: OCR分析区域对 {img_path.name} 无效。将使用完整帧。")
                    ...
                    if ocr_results and ocr_results[0]: # 检查结果是否有效
                        current_raw_lines = [item[1][0] for item in ocr_results[0] if item and len(item) > 1 and len(item[1]) > 0]
                    
                    # --- 预处理OCR结果 ---
                    current_processed_lines = self._preprocess_ocr_lines(current_raw_lines)
                    ...
                    if not kept_images: # 如果是第一张有效帧
                        ...
                        # 定义上一保留帧的尾部和当前帧的头部
                        tail_of_last_kept = last_kept_processed_lines_list[-self.overlap_check_tail_lines:]
                        head_of_current = current_processed_lines[:self.overlap_check_head_lines]
                        
                        # 检查是否存在重叠
                        has_overlap = self._lines_overlap(tail_of_last_kept, head_of_current)
                        
                        # 检查当前帧是否包含上一保留帧中没有的新内容行
                        has_new_overall_content = False
                        new_content_example = "" # 用于日志记录的新内容示例
                        for line in current_processed_lines:
                            if line not in last_kept_processed_lines_set: # line 已经是处理过的
                                has_new_overall_content = True
                                new_content_example = line
                                break
                        
                        # ---- 核心保留逻辑 ----
                        # 要保留后续帧，它必须与前一个保留的帧重叠，
                        # 并且必须引入前一个保留的帧中未见过的新内容。
                        if has_overlap and has_new_overall_content:
                            should_keep = True
                            self._log(f"保留: {img_path.name} (与上一保留帧重叠)")
                        else:
                            skip_reason = []
                            if not has_overlap:
                                skip_reason.append(f"无足够重叠")
                            if not has_new_overall_content:
                                skip_reason.append("无新内容")
                            self._log(f"跳过: {img_path.name} ({'; '.join(skip_reason)})")

                    if should_keep:
                        kept_images.append(str(img_path)) # 添加到保留列表
                        # 更新上一保留帧的行列表和行集合
                        last_kept_processed_lines_list = current_processed_lines
                        last_kept_processed_lines_set = set(current_processed_lines)

                except Exception as ocr_err: # 单个帧OCR处理的异常捕获
        ...
        self._progress(total_files, total_files) # 确保进度达到100%
        return kept_images
...
class PdfGenerator:
    ...
    def generate_pdf(self) -> Tuple[bool, str]:
        ...
        self._log(f"开始生成PDF: {output_pdf_path_obj.name} (布局: {self.layout}, {self.images_per_col}行x{self.images_per_row}列)")
        ...
        try:
            ...
            cell_padding = 1.5 * mm # 单元格内边距，稍微增加
            cell_total_width = content_width / self.images_per_row # 每个单元格的总宽度
            cell_total_height = content_height / self.images_per_col # 每个单元格的总高度
            
            # 确保容器尺寸为正
            img_container_width = max(1*mm, cell_total_width - 2 * cell_padding) # 图片容器宽度
            img_container_height = max(1*mm, cell_total_height - 2 * cell_padding) # 图片容器高度
            ...
            img_style = TableStyle([
                ('GRID', (0,0), (-1,-1), 0.5, reportlab_colors.lightgrey), # 网格线
            ...
            col_widths = [cell_total_width] * self.images_per_row # 每列宽度
            row_heights = [cell_total_height] * self.images_per_col # 每行高度
            ...
            for page_num in range(num_pages):
                ...
                page_image_paths = self.image_paths[start_idx:end_idx] # 当前页的图片路径
                if not page_image_paths: continue # 如果没有图片则跳过
            ...
            # 如果需要添加页眉页脚或标题，可以在 build 之前或使用更复杂的 Frame/PageTemplate 结构
            doc.build(story)
